...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    chars.groupBy(ch => ch).map(t => (t._1, t._2.length)).toList
  }
  ...
  def insert(fork: Fork, trees: List[CodeTree]): List[CodeTree] = {
    val weight = trees.head match {
      case x: Fork => x.weight
      case x: Leaf => x.weight
    }

    if (fork.weight <= weight) fork :: trees
  ...
  /**
    ...
    */
  def until(nonSingleTree: List[CodeTree] => Boolean, combineTwoTrees: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    if (nonSingleTree(trees)) trees
    else until(nonSingleTree, combineTwoTrees)(combineTwoTrees(trees))
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def traverse(ch: Char, tree: CodeTree): List[Bit] = {
      tree match {
        case Fork(left, right, _, _) => {
          if (chars(left).contains(ch)) 0 :: traverse(ch, left)
          else 1 :: traverse(ch, right)
        }
        case Leaf(c, _) => {
          List()
        }
    ...
    text.flatMap(char => traverse(char, tree))
  }
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    a.map(tuple => (tuple._1, 0 :: tuple._2)) ::: b.map(tuple => (tuple._1, 1 :: tuple._2))
  }
